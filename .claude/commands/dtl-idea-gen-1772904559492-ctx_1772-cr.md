...
Found 17 active idea(s) (24 total, excluding rejected/implemented for brevity):

### Pending Ideas (7)
1. **Custom styled intensity slider for EmotionalMarkers** (ui)
   - EmotionalMarkers.tsx uses a native HTML range input with an inline style gradient for the emotion intensity slider. Replace it with a custom-styled sl...
2. **Shared Themed Range Slider for Intensity Controls** (ui)
   - EmotionalMarkers IntensitySlider (line 179) uses a raw input[type=range] with inline background gradient and no webkit-appearance override. The same p...
3. **View Switcher with Animated Pill Indicator** (ui)
   - Replace the 5 discrete Button toggles in BeatsOverview (lines 451-503) with a segmented control that uses a Framer Motion layoutId sliding pill as the...
4. **Living Narrative Canvas � Spatial Story Worldbuilding** (ui)
   - Transform the NarrativeMap from a simple 4-column grid of beat cards into an infinite spatial canvas where the entire story lives as an interactive, z...
5. **Audience Simulation Engine � Test Stories on AI Readers** (functionality)
   - Build an audience simulation system that lets writers test their story structure on simulated readers before writing a single scene. Using the beat se...
6. **Predictive Narrative Physics � Real-Time Story Impact** (functionality)
   - Transform the beat system from a passive organizer into an active narrative physics engine that predicts consequences in real-time. The DependencyMana...
7. **Universal Story Genome � Structure DNA That Learns** (functionality)
   - Build a story genome system that encodes any narrative's structural DNA as a transferable, evolvable template. Today StructureAnalyzer has 6 hardcoded...

### Accepted Ideas (10)
1. **Beat cards view drag reorder with position persist** (functionality)
   - BeatsOverview cards view renders BeatCard components in a static grid with no reorder capability, unlike the table view which supports drag-and-drop v...
2. **Replace JS resize listener with CSS container query** (ui)
   - BeatFilterPanel.tsx lines 34-39 use a window.innerWidth resize listener with useState to toggle mobile layout at 768px. Replace this with a CSS contai...
3. **Differentiate function tag bar colors in chart** (ui)
   - In DistributionChart.tsx line 531, all function tag horizontal bars use a hardcoded uniform cyan (#06B6D4) via colorFn={() => "#06B6D4"} while categor...
4. **Fix view toggle keyboard shortcut order mismatch** (ui)
   - In BeatsOverview.tsx the visual tab order is Overview/Cards/Map/Taxonomy/Dependencies but keyboard shortcuts 1-5 map to a different sequence (1=table,...
5. **Standardize interactive state classes for all beats UI** (ui)
   - Create shared Tailwind utility classes (beat-interactive, beat-card, beat-row) that unify hover, active, and focus-visible states across all 51 files....
6. **Sticky Table Headers & View-Switching Shortcuts** (ui)
   - Add position: sticky with top: 0 and z-10 to the BeatsTable header row so column labels remain visible when scrolling through 20+ beats. Apply bg-slat...
7. **Migrate Drag-and-Drop to @dnd-kit with Drop Zones** (functionality)
   - Replace the deprecated @hello-pangea/dnd library (fork of react-beautiful-dnd) used in DraggableBeatRow.tsx with @dnd-kit, which provides built-in key...
8. **ARIA & Keyboard Accessibility for Beats Interactions** (ui)
   - Add role='progressbar' with aria-valuenow/min/max to the beat completion progress bar in BeatsOverview.tsx (currently a visual-only div). Add aria-liv...
9. **Standardize Error Handling to Toast + Retry Pattern** (ui)
   - The Beats system uses 4 conflicting error patterns: toast notifications (BeatsTable.tsx), inline red text (BeatsTableAdd.tsx), alert() dialogs (ActRec...
10. **AI Story Architect � LLM-Powered Beat Intelligence** (user_benefit)
   - Replace the entire keyword-based classification layer (TaxonomyLibrary's classifyBeatContent, suggestFunctions, analyzeEmotions) with deep LLM underst...
...

{
  ...
  "project_id": "0e9d4601-bf8c-43fe-b3e7-e0e9e2598b32",
  "context_id": "ctx_1772216016721_melxoel",
