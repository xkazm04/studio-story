...
Found 8 active idea(s) (11 total, excluding rejected/implemented for brevity):

### Pending Ideas (7)
1. **Export dialog live preview pane** (ui)
   - ExportDialog has a showPreview state variable (line 287) that is never used � there is no preview UI. Add a split-pane preview that renders a miniatur...
2. **Voice assigner with audio waveform preview badges** (ui)
   - VoiceAssigner VoiceCards show only text metadata (name, gender, age, style tags) with a tiny play button. Replace the play button with a mini waveform...
3. **Waveform display with real audio data visualization** (ui)
   - AudioTimeline uses generateMockWaveform() (line 102) with Math.random() to create fake waveform bars even when real audioData exists. Replace mock gen...
4. **Reading-mode typography with proper screenplay spacing** (ui)
   - ActItemStudio and SceneItemStudio render screenplay content using generic text-sm classes with no typographic hierarchy between scene headers, action ...
5. **Export progress indicator with success animation** (ui)
   - Add a multi-step progress indicator to ExportDialog showing: Preparing, Formatting, Generating, Ready. Replace the single isExporting boolean with a s...
6. **Canvas-based waveform renderer for AudioTimeline** (ui)
   - Replace the per-bar div rendering in WaveformDisplay (AudioTimeline line 180-218) with a single HTML5 canvas element. Draw waveform bars using canvas ...
7. **Export progress indicator with format-specific preview** (ui)
   - Add a multi-step progress indicator to ExportDialog that shows: (1) Preparing content, (2) Formatting for target, (3) Generating file, (4) Ready to do...

### Accepted Ideas (1)
1. **ScriptStatistics with act-level sparkline breakdown** (functionality)
   - ScriptStatistics renders 4 flat stat cards (words, dialogue, location, content) as equal-weight grid items with no trend or distribution information. ...
...

{
  ...
  "project_id": "0e9d4601-bf8c-43fe-b3e7-e0e9e2598b32",
  "context_id": "ctx_1772216032809_utjlmk3",
