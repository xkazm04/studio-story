...
### Pending Ideas (3)
1. **Event-sourced scene history with temporal queries** (functionality)
   - Replace the non-functional version history (ContextPanel.tsx history tab renders empty state, recordContextSnapshot callback is a no-op) with an event...
2. **Unified structured document model for scene content** (functionality)
   - Replace the two parallel editor implementations � ContentSection.tsx (plain textarea with string fields) and SceneEditorPanel.tsx (custom @block markd...
3. **CRDT-based collaborative scene editing engine** (functionality)
   - Replace the plain textarea + debounced full-document save (ContentSection.tsx 1500ms debounce replacing entire scene on every keystroke batch) with a ...

### Accepted Ideas (1)
1. **WebWorker NLP pipeline for scene analysis** (performance)
   - Move the synchronous presenceDetector.analyzePresence() call (PresenceTracker.tsx useMemo) and the format-mode transformers (ScreenplayFormatter, Pros...
...

{
  ...
  "project_id": "0e9d4601-bf8c-43fe-b3e7-e0e9e2598b32",
  "context_id": "ctx_1772216063342_ylgfzlm",
