...
### Pending Ideas (3)
1. **Voice-driven story dictation with live transcription** (functionality)
   - Extend voice mode beyond workspace commands into a full story dictation pipeline where writers speak narrative content and the system transcribes, str...
2. **Unified advisor context across voice and text modes** (functionality)
   - Merge the completely separate voice and text advisor channels into a unified conversation context that persists across mode switches. Currently useAdv...
3. **Full-duplex voice with VAD and automatic barge-in** (functionality)
   - Replace the push-to-talk architecture (manual startRecording/stopRecording in useAdvisorVoice.ts:219-252) with full-duplex continuous audio streaming ...

### Accepted Ideas (1)
1. **Resilient voice sessions with token refresh loop** (functionality)
   - Replace the single-use ephemeral token (live-token/route.ts:122 uses:1, 2-minute newSessionExpireTime) with a proactive token refresh mechanism that k...
...

{
  ...
  "project_id": "0e9d4601-bf8c-43fe-b3e7-e0e9e2598b32",
  "context_id": "ctx_1772216181553_pdve19t",
