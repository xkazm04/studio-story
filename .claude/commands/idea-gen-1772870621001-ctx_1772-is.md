...
### Pending Ideas (3)
1. **Learned Option Catalog from Generation History** (functionality)
   - Replace the 120+ hardcoded options in options.ts with a database-backed catalog that learns from generation history. Track which option combinations p...
2. **SSE Generation Pipeline Replaces Poll Chains** (performance)
   - Replace the recursive pollGeneration() setTimeout loop with a server-sent events stream from a new /api/ai/generation-stream endpoint. Currently each ...
3. **Semantic Prompt Graph Replaces String Concat** (functionality)
   - Replace selectComposedPrompt() string concatenation with a weighted graph where each category selection is a node with semantic relationships. The gra...

### Accepted Ideas (1)
1. **Reference Image Conditioning for Consistency** (functionality)
   - Add img2img and ControlNet reference image support to the generation pipeline, allowing users to condition new generations on existing character image...
...

{
  ...
  "project_id": "0e9d4601-bf8c-43fe-b3e7-e0e9e2598b32",
  "context_id": "ctx_1772216249295_zddl5cq",
