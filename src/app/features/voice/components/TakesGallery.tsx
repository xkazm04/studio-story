...
import WaveformVisualizer from '@/app/sound-lab/components/shared/WaveformVisualizer';
                ...
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    ...
                    className={cn(
                      ...
                      (take.rating ?? 0) >= star ? 'text-amber-400' : 'text-slate-400 hover:text-amber-300'
                    )}
                  >
                    <Star className="w-4 h-4" fill={(take.rating ?? 0) >= star ? 'currentColor' : 'none'} />
                  </button>
                ))}
              </div>
