    ...
    return (
      <div className={`p-4 ${className}`}>
        <SectionWrapper borderColor="gray" padding="md" className="text-center">
          ...
          <p className="text-sm text-slate-400">Select a project to use AI Assistant</p>
        </SectionWrapper>
      </div>
    );
  }
      ...
      <div className="p-3 border-b border-slate-700">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            <Sparkles className={`w-5 h-5 ${isEnabled ? 'text-purple-400' : 'text-slate-400'}`} />
            <h3 className={TYPOGRAPHY.h1}>AI Assistant</h3>
            {!isHealthy && (
              <span className={cn(TYPOGRAPHY.caption, 'text-red-400')} title="AI service unavailable">
                (Offline)
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className={`p-1.5 rounded transition-colors ${
                isEnabled ? 'bg-purple-600 text-white' : 'bg-slate-700 text-slate-400'
              }`}
        ...
        {!collapsed && isEnabled && (
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 hover:bg-purple-700 disabled:bg-slate-700 disabled:cursor-not-allowed rounded text-sm text-white transition-colors flex-1"
        ...
        <AnimatePresence>
          {showFilters && !collapsed && (
            <motion.div
              ...
              className="mt-3 p-3 bg-slate-800/50 rounded border border-slate-700"
            >
              <p className={cn(TYPOGRAPHY.h3, 'mb-2')}>Suggestion Types:</p>
              ...
              <p className={cn(TYPOGRAPHY.h3, 'mb-2 mt-3')}>Depth:</p>
            ...
            >
              <div className="space-y-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-slate-300">Auto-suggest on context change</span>
                </label>

                <div>
                  <label className="text-sm text-slate-400 block mb-1">Max Suggestions:</label>
                  <input
                    type="number"
                    min="1"
                    max="10"
                    ...
                    className="w-full px-2 py-1 text-sm bg-slate-700 border border-slate-600 rounded text-white focus:ring-1 focus:ring-purple-500"
                    data-testid="max-suggestions-input"
                  />
                </div>

                <div>
                  <label className="text-sm text-slate-400 block mb-1">Genre Filter (optional):</label>
                  <input
                    type="text"
                    value={settings.genre_filter || ''}
                    onChange={(e) => updateSettings({ genre_filter: e.target.value })}
                    placeholder="e.g., sci-fi, fantasy..."
                    className="w-full px-2 py-1 text-sm bg-slate-700 border border-slate-600 rounded text-white placeholder-slate-500 focus:ring-1 focus:ring-purple-500"
