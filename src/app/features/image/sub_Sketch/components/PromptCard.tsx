    ...
    >
      ...
      <div className="flex items-start gap-2 mb-1.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p
            className={clsx(
              "text-sm leading-snug mt-0.5 line-clamp-2 transition-colors",
              isSelected ? "text-slate-300" : "text-slate-400 group-hover:text-slate-400"
            )}
          >
            {option.description}
          </p>
        ...
        <div className="flex flex-wrap gap-1 mt-1.5">
          {option.tags.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className={clsx(
                "px-1.5 py-0.5 rounded text-sm font-medium transition-all",
                isSelected
                  ? `${styles.bg} ${styles.text}`
                  : "bg-slate-900/60 text-slate-400 group-hover:bg-slate-800/80 group-hover:text-slate-400"
              )}
            >
              {tag}
            </span>
