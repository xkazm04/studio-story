...
const CATEGORY_CONFIG: Record<AccessoryCategory, { label: string; icon: React.ReactNode; color: string }> = {
  ...
  document: { label: 'Document', icon: <FileText size={16} />, color: 'text-slate-400 bg-slate-600/20' },
  currency: { label: 'Currency', icon: <Coins size={16} />, color: 'text-yellow-400 bg-yellow-600/20' },
  other: { label: 'Other', icon: <MoreHorizontal size={16} />, color: 'text-slate-400 bg-slate-600/20' },
};
  ...
  if (viewMode === 'list') {
    return (
      <motion.div
        ...
        className="flex items-center gap-4 p-3 bg-slate-800/50 border border-slate-700 rounded-lg hover:border-slate-600 transition-colors"
      >
        {/* Icon */}
  ...
  return (
    ...
    >
      ...
      <div className="cursor-pointer" onClick={onClick}>
        ...
        <p className="text-sm text-slate-400 mb-2">{categoryConfig.label}</p>
      ...
      {accessory.story_significance && (
        <div className="p-2 bg-purple-900/20 border border-purple-700/30 rounded text-sm text-purple-300 mb-3">
      ...
      <div className="flex items-center gap-2 pt-2 border-t border-slate-700">
        <button
          onClick={onEdit}
          className="flex-1 flex items-center justify-center gap-1 px-3 py-1.5 text-sm text-slate-400 hover:text-white hover:bg-slate-700 rounded transition-colors"
        >
          <Edit3 size={14} />
          Edit
        </button>
        <button
          onClick={onDelete}
          className="flex items-center justify-center gap-1 px-3 py-1.5 text-sm text-slate-400 hover:text-red-400 hover:bg-red-900/20 rounded transition-colors"
        >
          <Trash2 size={14} />
        </button>
      </div>
...
const StateDropdown: React.FC<StateDropdownProps> = ({ currentState, onSelect, onClose }) => {
  return (
    <>
      ...
      <div className="absolute right-0 top-full mt-1 w-32 bg-slate-800 border border-slate-700 rounded-lg shadow-lg overflow-hidden z-20">
        {(Object.keys(STATE_CONFIG) as AccessoryState[]).map((state) => (
          <button
            ...
            className={cn(
              ...
              currentState === state ? 'bg-slate-700' : ''
            )}
          >
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          className={cn(
            'w-full px-3 py-2 bg-slate-800 border rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500',
        ...
        <div className="grid grid-cols-4 gap-2">
          {(Object.keys(CATEGORY_CONFIG) as AccessoryCategory[]).map((category) => (
            <button
              ...
              className={cn(
                'flex flex-col items-center gap-1 p-2 rounded-lg border text-sm transition-all',
      ...
      <div>
        ...
        <textarea
          ...
          rows={3}
          className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none"
      ...
      {/* State */}
      <div>
        <label className="block text-sm font-medium text-slate-300 mb-1">Current State</label>
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400">Mark as a defining item for this character</p>
        </div>
      </div>

      {/* Story Significance */}
      <div>
        <label className="block text-sm font-medium text-slate-300 mb-1">Story Significance</label>
        <textarea
  ...
  return (
    <div className="mb-4">
      <button
        ...
        className="w-full flex items-center gap-2 px-3 py-2 bg-slate-800/50 rounded-lg hover:bg-slate-800 transition-colors"
      >
        <div className={cn('p-1.5 rounded', categoryConfig.color)}>
          {categoryConfig.icon}
        </div>
        <span className="font-medium text-white">{categoryConfig.label}</span>
        <span className="text-sm text-slate-400">({accessories.length})</span>
        {isExpanded ? (
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
          ...
          {showForm && (
            ...
            >
              <motion.div
                ...
                className="w-full max-w-lg max-h-[90vh] overflow-y-auto bg-slate-900 border border-slate-700 rounded-xl p-6"
              >
                <div className="flex items-center justify-between mb-4">
