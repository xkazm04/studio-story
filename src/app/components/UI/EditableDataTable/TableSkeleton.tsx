...
export function TableSkeleton({ columns, rows = 5, className }: TableSkeletonProps) {
  return (
    <div className={clsx('w-full overflow-hidden rounded-lg border border-slate-800', className)}>
      {/* Header Skeleton */}
      <div className="flex py-2 px-3 border-b bg-slate-900/50 border-slate-800">
      ...
      <div className="bg-slate-950">
        {Array.from({ length: rows }).map((_, rowIdx) => (
          <div key={rowIdx} className="flex items-center py-2 px-3 border-b border-slate-800/50">
