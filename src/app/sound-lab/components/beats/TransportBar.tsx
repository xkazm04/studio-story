...
export default function TransportBar({
  ...
  return (
    ...
    >
      ...
      {isPlaying && (
        <span
          className="text-sm text-amber-400 font-mono font-semibold"
      ...
      <div className="flex items-center gap-1.5">
        ...
        <input
          ...
          className="w-14 px-2 py-1 bg-slate-950/60 border border-slate-700/30 rounded-md text-sm text-slate-200
      ...
      <div className="flex items-center gap-1.5">
        ...
        />
        <span className="text-sm text-slate-400 font-mono w-7">{Math.round(pattern.swing * 100)}%</span>
      </div>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <select
          ...
          className="px-2 py-1 bg-slate-950/60 border border-slate-700/30 rounded-md text-sm text-slate-200
            ...
            {availableInstruments.map((inst) => {
              ...
              return (
                <button
                  ...
                  className="w-full flex items-center gap-2 px-3 py-1.5 text-sm text-slate-300 hover:bg-slate-800/40 transition-colors"
                >
          ...
          >
            {MOCK_BEAT_PATTERNS.map((preset, i) => (
              <button
                ...
                className="w-full px-3 py-1.5 text-left text-sm text-slate-300 hover:bg-slate-800/40 transition-colors"
              >
                <span className="font-medium block">{preset.name}</span>
                <span className="text-sm text-slate-400 font-mono">{preset.bpm} BPM / {preset.genre}</span>
              </button>
            ))}
            <div className="border-t border-slate-700/20" />
            <button
              onClick={() => { onClear(); setShowPresets(false); }}
              className="w-full flex items-center gap-1.5 px-3 py-1.5 text-left text-sm text-slate-400 hover:text-slate-200 hover:bg-slate-800/40 transition-colors"
            >
      ...
      <button
        onClick={onSave}
        disabled={isSaving}
        className="flex items-center gap-1 text-sm text-slate-400 hover:text-orange-300 hover:bg-orange-500/10 px-1.5 py-1 rounded-md transition-all disabled:opacity-50"
