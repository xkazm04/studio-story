    ...
    <div className={cn(
      ...
      <div className="p-3">
        ...
        <div className="flex items-center gap-2 mb-2">
          <span className={cn(
            'text-sm px-2 py-0.5 rounded font-medium border-l-4',
        ...
        {/* Description */}
        <p className="text-sm text-slate-300 leading-relaxed mb-2">{suggestion.description}</p>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 h-9 px-3 bg-slate-900/60 border-b border-slate-700/40">
        ...
        <span className="text-sm font-semibold text-slate-200">AI Director</span>
      ...
      <div className="p-3 space-y-3">
        {/* Scene Selector */}
        <div>
          <span className="text-sm text-slate-400 font-medium mb-1 block">Scene Context</span>
          <div className="relative">
            <button
              onClick={() => setIsOpen(!isOpen)}
              className="w-full flex items-center justify-between px-3 py-2 rounded-md
                border border-slate-700/50 bg-slate-900/60 text-sm text-slate-300
            ...
            {isOpen && (
              <div className="absolute top-full left-0 right-0 mt-1 z-20 rounded-md border border-slate-700/50 bg-slate-900 shadow-xl shadow-black/30">
                {MOCK_SCENES.map((s) => (
                  ...
                  >
                    <span className="text-sm font-medium text-slate-300 block">{s.name}</span>
                    <span className="text-sm text-slate-400">{s.mood} / {s.characters.join(', ')}</span>
                  </button>
                ))}
              </div>
            )}
          ...
          {scene && (
            <div className="mt-2 px-3 py-2 rounded bg-slate-900/30 border border-slate-800/30">
              <p className="text-sm text-slate-400 leading-relaxed">{scene.setting}</p>
            </div>
          )}
        </div>
        ...
        {isThinking && (
          <div className="flex items-center justify-center py-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-orange-400">Analyzing scene context...</span>
            </div>
          </div>
        )}
        ...
        {!isThinking && suggestions.length === 0 && !selectedScene && (
          <div className="flex items-center justify-center py-6">
            <span className="text-sm text-slate-400">Select a scene to get AI audio suggestions</span>
          </div>
        )}
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 pt-1">
              ...
              <button
                ...
                className="flex items-center gap-1 ml-auto px-2.5 py-1 rounded text-sm font-medium
