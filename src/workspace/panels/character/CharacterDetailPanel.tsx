...
export default function CharacterDetailPanel({
  ...
  return (
    <PanelFrame title="Character Detail" icon={User} onClose={onClose} headerAccent="cyan" density={density}>
      {id ? (
        <CharacterDetails characterId={id} />
      ) : (
        <div className="flex flex-col items-center justify-center h-full p-6 text-center">
          <User className="w-8 h-8 text-slate-400 mb-3" />
          <p className="text-sm text-slate-400">Select a character to view details</p>
          {onTriggerSkill && (
            <button
              onClick={() => onTriggerSkill('character-backstory')}
              className="mt-3 px-3 py-1 text-sm font-medium text-purple-400 border border-purple-500/30 rounded hover:bg-purple-500/10 transition-colors"
            >
              Generate Character
            </button>
          )}
